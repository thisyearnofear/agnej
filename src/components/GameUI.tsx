...
export default function GameUI({
  ...
  const toggleRules = () =>
    setShowRules
      ...
      : setLocalShowRules(!localShowRules);
  const closeRules = () =>
    setShowRules ? setShowRules(false) : setLocalShowRules(false);
  ...
  const uiOpacity =
    gameState === "ACTIVE" && uiManuallyHidden ? "opacity-10" : "opacity-100";
  ...
  return (
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-4 md:p-6">
      {/* Top Bar: Pot & Status */}
      <div
        className={`flex justify-between items-start pointer-events-auto ${uiTransition} ${uiOpacity}`}
      >
        <div className="flex gap-4">
          ...
          {gameState === "ACTIVE" && (
            <button
              ...
              title={
                !uiManuallyHidden ? "UI Always Visible" : "Show UI Controls"
              }
            >
              {!uiManuallyHidden ? (
          ...
          {score !== undefined && (
            <div className="flex gap-4">
              <div
                className={`bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-4 text-white relative transition-transform duration-200 ${scoreJuice ? "scale-110 border-yellow-400/50" : ""}`}
              >
                <div className="text-xs text-gray-400 uppercase tracking-wider font-bold">
                  Score
                </div>
                <div className="text-2xl font-mono text-yellow-400 flex items-center gap-2">
                  {score}
                  {scoreJuice && (
                    <span className="text-sm text-yellow-200 animate-bounce absolute -right-2 -top-2">
                      +1
                    </span>
                  )}
                </div>
        ...
        <div className="flex flex-col gap-2 items-end">
          <div
            className={`bg-black/40 backdrop-blur-md border rounded-xl p-4 text-white text-right transition-all ${
              gameState === "ACTIVE"
                ? "border-blue-400/50"
                : gameState === "VOTING"
          ...
          >
            ...
            <div
              className={`text-lg font-bold flex items-center justify-end gap-2 ${
                gameState === "ACTIVE"
                  ? "text-blue-400"
                  : gameState === "VOTING"
                    ? "text-yellow-400"
          ...
          {gameState === "ACTIVE" && (
            <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-2 text-white w-48">
              <div className="flex justify-between text-xs mb-1">
                ...
                <span
                  className={`${stability < 30 ? "text-red-500" : "text-green-500"}`}
                >
      ...
      {gameState === "ACTIVE" &&
        timeLeft !== undefined &&
        (gameMode === "SOLO_COMPETITOR" || gameMode === "MULTIPLAYER") && (
          <div
            className={`absolute top-24 left-1/2 -translate-x-1/2 pointer-events-none flex flex-col items-center gap-2 ${uiTransition} ${uiOpacity}`}
          >
            <div
              className={`text-4xl font-black drop-shadow-lg transition-colors duration-300 ${
      ...
      <div
        className={`flex flex-col md:flex-row gap-4 items-end pointer-events-auto ${uiTransition} ${uiOpacity}`}
      >
        {/* Player List */}
          ...
          <div className="space-y-2">
            {players.map((player) => (
              ...
              >
                ...
                {player.isCurrentTurn && (
                  <span className="text-blue-400 text-xs font-bold">TURN</span>
                )}
                {!player.isAlive && (
        ...
        <div className="flex-1 flex justify-end gap-3">
          {gameState === "WAITING" && (
            <div className="flex flex-col gap-3 items-end">
                ...
                <div className="text-sm text-gray-300 mb-2">
                  {isPractice ? (
                    <span className="text-purple-300">
                      Practice Mode • No Stake
                    </span>
                  ) : (
                    <>
                      <div>{difficulty} Difficulty</div>
                      <div className="text-green-400 font-semibold">
                        {stake} USDC per player
                      </div>
                    </>
                  )}
                </div>
              ...
              <button
                onClick={onJoin}
                className="bg-linear-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white px-8 py-4 rounded-xl font-bold shadow-lg shadow-blue-900/30 transition-all active:scale-95 hover:scale-105 flex items-center gap-2"
              >
                <span>▶</span>
          ...
          {gameState === "VOTING" && (
            <div className="flex flex-col gap-2 bg-linear-to-br from-yellow-900/40 to-red-900/40 p-5 rounded-xl border-2 border-yellow-400/60 shadow-lg shadow-yellow-500/20 animate-fadeIn">
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => onVote(true)}
                  className="flex-1 bg-linear-to-b from-green-600 to-green-700 hover:from-green-500 hover:to-green-600 text-white px-6 py-4 rounded-xl font-bold shadow-lg shadow-green-900/40 transition-all active:scale-95 hover:scale-105 flex items-center justify-center gap-2"
                >
                  <span>💚</span>
                  <span>Split Pot</span>
                </button>
                <button
                  ...
                  className="flex-1 bg-linear-to-b from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white px-6 py-4 rounded-xl font-bold shadow-lg shadow-red-900/40 transition-all active:scale-95 hover:scale-105 flex items-center justify-center gap-2"
                >
                  <span>🔥</span>
                  <span>Continue</span>
                </button>
      ...
      <div className="pointer-events-auto">
        ...
        {isRulesVisible && (
          <div
            className="absolute inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-2 md:p-4"
            onClick={closeRules}
          >
            <div
              className="bg-linear-to-br from-gray-900 to-gray-800 border-2 border-yellow-500/50 p-4 md:p-8 rounded-2xl max-w-md w-full shadow-2xl max-h-[90vh] overflow-y-auto"
              ...
              {isMobile && gameState === "ACTIVE" && (
                <div className="mb-3 bg-yellow-500/20 border border-yellow-500/50 rounded-lg p-2 text-center">
                  <p className="text-yellow-300 text-xs font-bold">
                    ⚠️ This dialog is blocking gameplay
                  </p>
              ...
              {gameMode === "SOLO_PRACTICE" ? (
                ...
                <div className="space-y-3 md:space-y-6">
                  <div className="flex gap-2 md:gap-4 items-start">
                    <div className="bg-green-500/20 p-2 md:p-3 rounded-lg text-green-400 shrink-0">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="20"
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-xs md:text-sm">
                        Experiment freely! No time limits, no locked layers, no
                        pressure. Perfect for learning the physics.
                      </p>
                    </div>
                  </div>

                  <div className="flex gap-2 md:gap-4 items-start">
                    <div className="bg-blue-500/20 p-2 md:p-3 rounded-lg text-blue-400 shrink-0">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="20"
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-xs md:text-sm">
                        ...
                        Experiment with different strategies and learn how
                        blocks interact.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 md:gap-4 items-start">
                    <div className="bg-purple-500/20 p-2 md:p-3 rounded-lg text-purple-400 shrink-0">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="20"
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-xs md:text-sm">
                        ...
                        Remove blocks from any layer, including the top. Explore
                        freely.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-2 md:p-4 mt-2 md:mt-4">
                    <p className="text-green-300 text-xs md:text-sm text-center">
                      💡 <span className="font-bold">Tip:</span> When
                      you&apos;re ready for a challenge, try{" "}
                      <span className="font-bold">Solo Competitor</span> mode!
                    </p>
                  </div>
                </div>
              ) : (
                ...
                <div className="space-y-3 md:space-y-6">
                  <div className="flex gap-2 md:gap-4 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-xs md:text-sm">
                        Remove blocks from the tower to score points. Each
                        removed block = 1 point.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 md:gap-4 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-xs md:text-sm">
                        The top 2 layers are{" "}
                        <span className="text-red-400 font-bold">LOCKED</span>.
                        You cannot remove blocks from these layers.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 md:gap-4 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-xs md:text-sm">
                        ...
                        to make a move. The timer resets after each successful
                        score.
                      </p>
                    </div>
                  </div>
