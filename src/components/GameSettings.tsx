...
export default function GameSettings({ onStart }: GameSettingsProps) {
  ...
  const [gameMode, setGameMode] = useState<
    "SOLO_PRACTICE" | "SOLO_COMPETITOR" | "SINGLE_VS_AI" | "MULTIPLAYER"
  >("SOLO_PRACTICE");
  ...
  const { lobbies, fetchLobbies, signAndConnect, authSignature } =
    useGameSocket();
  ...
  const handleModeSelect = (
    mode: "SOLO_PRACTICE" | "SOLO_COMPETITOR" | "SINGLE_VS_AI" | "MULTIPLAYER",
  ) => {
    setGameMode(mode);
  ...
  const getStepDescription = () => {
    switch (currentStep) {
      case "mode":
        return "How would you like to play?";
      case "config":
        return "Customize your game settings";
      case "auth":
        return "Sign a message to authenticate";
      case "summary":
        return "Review and start your game";
    }
  };
    ...
    const settings: GameSettingsConfig = {
      ...
      playerCount:
        gameMode === "SOLO_PRACTICE" || gameMode === "SOLO_COMPETITOR"
          ...
          : playerCount,
      aiOpponentCount:
        gameMode === "SINGLE_VS_AI" ? aiOpponentCount : undefined,
      difficulty,
      stake:
        gameMode === "SOLO_PRACTICE" || gameMode === "SOLO_COMPETITOR"
          ? 0
        ...
        {[
          ...
          >
            <div className="flex items-start gap-2 md:gap-4">
              <span
                className={`text-2xl md:text-4xl transition-transform duration-300 ${mode.disabled ? "grayscale" : "group-hover:scale-110"}`}
              >
                {mode.icon}
              </span>
    ...
    <div className="space-y-6">
      ...
      {gameMode === "SINGLE_VS_AI" && (
        <div>
          <label className="block text-white font-semibold mb-2 text-sm">
            AI Opponents:{" "}
            <span className="text-blue-400">{aiOpponentCount}</span>
          </label>
        ...
        <div>
          ...
          {isHost && (
            <>
              <label className="block text-white font-semibold mb-2 text-sm">
                Max Players:{" "}
                <span className="text-blue-400">{playerCount}</span>
              </label>
              <div className="flex gap-2">
                ...
                {lobbies.length === 0 ? (
                  <div className="text-gray-500 text-sm text-center py-4 bg-white/5 rounded-lg border border-white/5">
                    ...
                    <span className="text-xs">
                      Click &quot;Join Game&quot; to auto-match or wait for one.
                    </span>
                  </div>
                ) : (
                  lobbies.map((lobby) => (
                    ...
                    >
                      <div className="text-left">
                        <div className="font-bold text-sm">
                          Game #{lobby.id.toString().slice(-4)}
                        </div>
                        <div className="text-xs text-gray-500">
                          {lobby.difficulty} • {lobby.stake} USDC
                        </div>
                      </div>
      ...
      {gameMode !== "SOLO_PRACTICE" && gameMode !== "SOLO_COMPETITOR" && (
        <div>
          <label className="block text-white font-semibold mb-2 text-sm">
            Stake per Player:{" "}
            <span className="text-green-400">{stake} USDC</span>
          </label>
          <div className="flex gap-2">
        ...
        <label className="flex items-center justify-between cursor-pointer group">
          <div>
            ...
            <div className="text-gray-500 text-xs">
              Show selection highlight & drag arrow
            </div>
          </div>
          <button
            type="button"
            className={`w-12 h-6 rounded-full p-1 transition-colors ${showHelpers ? "bg-green-600" : "bg-gray-700"}`}
            onClick={() => setShowHelpers(!showHelpers)}
          >
            <div
              className={`w-4 h-4 rounded-full bg-white transition-transform ${showHelpers ? "translate-x-6" : "translate-x-0"}`}
            />
          </button>
        </label>
      </div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-gray-300 text-sm mb-4">
              Sign a message with your wallet to verify your identity on the
              game server. This is a one-time authentication that will be saved
              locally.
            </p>
            <p className="text-gray-400 text-xs">
              <strong>What happens:</strong> You&apos;ll be asked to sign a
              message (no gas fees). This proves you control the wallet and
              prevents spoofing.
            </p>
          </div>
        </div>
        ...
        {authSignature ? (
          ...
          <ButtonWithFeedback
            onClick={() => {
              ...
              void signAndConnect().finally(() => setIsSigningAuth(false));
            }}
        ...
        <div className="space-y-4">
            ...
            <div>
              ...
              <div className="text-sm text-gray-400">
                {gameMode === "SOLO_PRACTICE"
                  ? "Free practice mode"
                  : gameMode === "SOLO_COMPETITOR"
                    ? `Ranked ${difficulty.toLowerCase()} difficulty`
            ...
            <div className="flex items-center gap-4 p-4 bg-green-500/10 rounded-xl border border-green-500/20">
              ...
              <div>
                <div className="font-bold text-green-400">
                  {getTotalPot().toFixed(2)} USDC Total Pot
                </div>
          ...
          <div className="flex items-center gap-4 p-4 bg-white/5 rounded-xl">
            ...
            <div>
              ...
              <div className="text-sm text-gray-400">
                {showHelpers
                  ? "Selection highlights and drag arrows will be shown"
                  : "Clean gameplay without visual aids"}
              </div>
            </div>
          </div>
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      ...
      <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl p-3 md:p-6">
        ...
        <div className="flex items-center gap-2 mb-3 md:mb-6">
          {(["mode", "config", "auth", "summary"] as const).map((step, idx) => (
            <div key={step} className="flex items-center gap-1 md:gap-2">
              <div
                className={`w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center text-xs md:text-sm font-bold transition-all ${
                  currentStep === step
                    ...
                    : (["mode", "config", "auth", "summary"] as const).indexOf(
                          currentStep,
                        ) > idx
              ...
              {idx < 3 && (
                <div
                  className={`w-4 md:w-8 h-1 rounded-full transition-all ${
                    (["mode", "config", "auth", "summary"] as const).indexOf(
                      currentStep,
                    ) > idx
                      ? "bg-green-600"
        ...
        <div className="mb-4 md:mb-6">{renderContent()}</div>
          ...
          <ButtonWithFeedback
            onClick={() => {
              if (currentStep === "config") setCurrentStep("mode");
                ...
                setCurrentStep(gameMode === "MULTIPLAYER" ? "auth" : "config");
            }}
            type="secondary"
