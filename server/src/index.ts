...
blockchain.listenToEvents({
  onPlayerJoined: (gameId, player) => {
    ...
    // Sync: Ensure the game instance exists when players join on-chain
    let game = gameManager.getGame(gameId);
    if (!game) {
      game = gameManager.createGame(
        {
          maxPlayers: 7, // Matches MAX_PLAYERS in HouseOfCards.sol
          difficulty: "MEDIUM",
          stake: 0.001, // Matches ENTRY_STAKE in HouseOfCards.sol
          isPractice: false,
        },
        gameId,
      );
  ...
  onGameStarted: (gameId) => {
    console.log(`[Blockchain] Game ${gameId} started on-chain`);
  },
  onTurnChanged: (gameId, player) => {
    console.log(
      `[Blockchain] Turn changed on-chain in game ${gameId}: ${player}`,
    );
  },
});
...
io.use((socket, next) => {
  ...
  if (!address || !signature || !message) {
    ...
    next(
      new Error(
        "Authentication required: Missing address, signature, or message",
      ),
    );
    return;
  }
...
io.on("connection", (socket) => {
  ...
  socket.on("joinGame", (data: unknown) => {
    ...
    if (typeof data === "string") {
      ...
      playerAddress = d.address;
      gameId = d.gameId;
      asSpectator = d.asSpectator || false;
    }

    // SECURITY: Enforce that joining address matches authenticated socket address
    if (
      socket.data.authenticatedAddress.toLowerCase() !==
      playerAddress.toLowerCase()
    ) {
      console.warn(
        `[Security] Spoof attempt? Socket auth: ${socket.data.authenticatedAddress}, claimed: ${playerAddress}`,
      );
  ...
  socket.on(
    "submitMove",
    (data: { blockIndex: number; force: unknown; point: unknown }) => {
      try {
