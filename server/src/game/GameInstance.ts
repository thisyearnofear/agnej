...
export class GameInstance extends EventEmitter {
  ...
  public getPublicState(): GameState {
    ...
    return {
      ...
      currentPlayerIndex:
        this.players.findIndex((p) => p === turnState?.currentPlayer) || 0,
      status: this.status,
  ...
  public addPlayer(
    ...
    if (!this.players.includes(playerAddress)) {
      ...
      if (this.status === "WAITING" && !asSpectator) {
        ...
        console.log(
          `[GameInstance] Player ${playerAddress} joined. Now ${this.players.length}/${this.maxPlayers}`,
        );
        ...
        const success = this.spectatorManager.addSpectator(
          playerAddress,
          socketId,
        );
        if (success) {
      ...
      const wasDisconnected =
        this.reconnectionManager.isDisconnected(playerAddress);
      if (wasDisconnected) {
        const timeRemaining =
          this.reconnectionManager.getTimeRemaining(playerAddress);
        if (timeRemaining > 0) {
          ...
          this.lastActivityTime = Date.now();
          console.log(
            `[GameInstance] Player ${playerAddress} reconnected within grace period`,
          );
          ...
          console.log(
            `[GameInstance] Player ${playerAddress} attempted reconnect but grace period expired`,
          );
          this.emit("playerReconnectFailed", {
            playerAddress,
            reason: "GRACE_PERIOD_EXPIRED",
          });
          // Emit to client
          this.io.to(this.roomId).emit("playerReconnectFailed", {
            playerAddress,
            reason: "GRACE_PERIOD_EXPIRED",
          });
          return false;
        ...
        !this.activePlayers.has(playerAddress) &&
        this.status !== "ENDED" &&
        ...
        this.lastActivityTime = Date.now();
        console.log(
          `[GameInstance] Player ${playerAddress} re-established connection`,
        );
        this.emit("playerReconnected", { playerAddress });
        this.broadcastState();
        return true;
  ...
  private shouldAutoStart(): boolean {
    ...
    return (
      this.activePlayers.size >= 2 || this.activePlayers.size >= this.maxPlayers
    );
  }
  ...
  private startGame(): void {
    ...
    // Oracle: Report game start
    if (!this.isPractice) {
      this.blockchain
        .startGame(this.id)
        .catch((err) =>
          console.error(
            `[Oracle] Failed to start game ${this.id} on-chain`,
            err,
          ),
        );
    }
    ...
    console.log(
      `[GameInstance] Game ${this.id} started with ${players.length} players`,
    );
  ...
  public removePlayer(
    ...
    console.log(
      `[GameInstance] Player ${playerAddress.slice(0, 6)}... ${reason} from game ${this.id}`,
    );
    this.activePlayers.delete(playerAddress);
    this.metrics.recordPlayerRemoved();
    this.lastActivityTime = Date.now();

    // Oracle: Report player elimination
    if (!this.isPractice) {
      this.blockchain
        .eliminatePlayer(this.id, playerAddress, reason)
        .catch((err) =>
          console.error(
            `[Oracle] Failed to eliminate player ${playerAddress} on-chain`,
            err,
          ),
        );
    }
  ...
  public handleMove(playerAddress: string, move: MoveData): void {
    ...
    if (!validation.isValid) {
      console.warn(
        `[GameInstance] Invalid move from ${playerAddress}:`,
   ...
   */
  private processTurnEnd(turnState: TurnState): void {
    if (!this.turnManager || this.status !== "ACTIVE") return;

    // Oracle: Report turn result
    if (!this.isPractice && turnState.currentPlayer) {
      // Only report if the player wasn't already eliminated via removePlayer
      // (If they were removed, activePlayers.has will be false)
      if (this.activePlayers.has(turnState.currentPlayer)) {
        if (turnState.moveSubmitted) {
          this.blockchain
            .completeTurn(this.id)
            .catch((err) =>
              console.error(
                `[Oracle] Failed to complete turn for game ${this.id}`,
                err,
              ),
            );
        } else {
          this.blockchain
            ...
            .catch((err) =>
              console.error(
                `[Oracle] Failed to timeout turn for game ${this.id}`,
                err,
              ),
            );
        }
  ...
  public update(dt: number) {
    ...
    for (const expiredPlayer of expiredPlayers) {
      console.log(
        `[GameInstance] Permanently removing ${expiredPlayer.address} - reconnection grace period expired`,
      );
  ...
  public exportMetrics() {
    const { memoryUsage, snapshotCount, eventCount } = this.getHistoryStats();
