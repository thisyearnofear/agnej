...
export class GameManager {
  ...
  public createGame(config: GameConfig, onChainId?: number): GameInstance {
    const id = onChainId || Date.now();
    const game = new GameInstance(id, config, this.io, this.blockchain);
    this.games.set(id, game);
    console.log(`[GameManager] Game ${id} created. (On-Chain: ${!!onChainId})`);
    return game;
  }
   ...
   * Get a game by ID
   */
  public getGame(gameId: number): GameInstance | undefined {
  ...
  public joinGame(
    ...
    if (gameId) {
      ...
      if (!game) {
        console.log(
          `[GameManager] No available games for ${playerAddress}. Player should create one.`,
        );
    ...
    if (!asSpectator && !game.isPractice && game.difficulty !== "EASY") {
      ...
      if (!hasPaid) {
        console.warn(
          `[GameManager] Access Denied: Player ${playerAddress} has not paid for game ${game.id}`,
        );
    ...
    if (!success) {
      ...
      const reason = asSpectator
        ? "Could not join as spectator."
        : "Could not join game. Game full or not accepting players.";
    ...
    console.log(
      `[GameManager] ${joinType} ${playerAddress} joined game ${game.id}`,
    );
    return game;
  }
  ...
  public getPublicLobbies() {
    const lobbies = [];
    for (const game of this.games.values()) {
      // Filter: Waiting, Not Practice, and (Multiplayer or On-chain)
      if (game.status === "WAITING" && !game.isPractice) {
  ...
  private tick() {
    ...
    for (const [id, game] of this.games.entries()) {
      ...
      if (
        (game.status === "ENDED" || game.status === "COLLAPSED") &&
        inactiveTime > 5 * 60 * 1000
      ) {
        console.log(
          `[GameManager] Cleaning up finished game ${id} (result retention expired)`,
        );
        shouldDelete = true;
      }
