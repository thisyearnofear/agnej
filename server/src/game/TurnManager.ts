...
export class TurnManager extends EventEmitter {
  ...
  public startTurn(): TurnState {
    ...
    console.log(
      `[TurnManager] Turn started for ${nextPlayer}, deadline in ${this.turnTimeoutMs}ms`,
    );
   ...
   */
  public submitMove(playerAddress: string, moveData: unknown): boolean {
    if (this.state.currentPlayer !== playerAddress) {
      console.warn(
        `[TurnManager] Invalid move: ${playerAddress} is not current player (${this.state.currentPlayer})`,
      );
    ...
    if (this.state.moveSubmitted) {
      console.warn(
        `[TurnManager] Invalid move: Move already submitted this turn`,
      );
      this.emit("moveRejected", {
        reason: "MOVE_ALREADY_SUBMITTED",
        playerAddress,
      });
      return false;
    }
  ...
  public removePlayer(playerAddress: string): void {
    ...
    console.log(
      `[TurnManager] Removed ${playerAddress}, ${this.activePlayers.length} players remain`,
    );
  }
